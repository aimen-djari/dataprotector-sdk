...
export function CreateNewContent() {
	...
	async function handleFile() {
		...
		try {
			...
			// --- New consume content
			await dataProtector.dataProtectorSharing.hostProtectedData({
				app: import.meta.env.VITE_PROTECTED_DATA_DELIVERY_DAPP_ADDRESS,
				protectedData: address,
				workerpool: import.meta.env.VITE_WORKERPOOL_ADDRESS,
				onStatusUpdate: (status) => {
					if (status.title === 'FETCH_WORKERPOOL_ORDERBOOK') {
						const title = 'Check for iExec workers availability';
						if (!status.isDone) {
							addOrUpdateStatusToStore({ title, isDone: false });
						} else {
							addOrUpdateStatusToStore({ title, isDone: true });
					...
					if (status.title === 'HOST_ORDER_REQUESTED') {
						const title = 'Request to access this content';
						if (!status.isDone) {
							addOrUpdateStatusToStore({ title, isDone: false });
						} else {
							addOrUpdateStatusToStore({ title, isDone: true });
					...
					if (status.title === 'HOST_TASK_ACTIVE') {
						const title = 'Content now being handled by iExec dApp';
						if (!status.isDone) {
							addOrUpdateStatusToStore({ title, isDone: false });
						} else {
							addOrUpdateStatusToStore({ title, isDone: true });
					...
					if (status.title === 'HOST_TASK_ERROR') {
						const title = 'An error occurred while hosting the task';
						if (!status.isDone) {
							addOrUpdateStatusToStore({ title, isDone: false });
						} else {
							addOrUpdateStatusToStore({ title, isDone: true });
